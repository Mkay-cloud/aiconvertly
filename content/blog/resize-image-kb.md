...
2. Upload your photo. It accepts JPEG, PNG, or WebP. ![uploading a photo to the Compress Image to KB tool](/blog/resize-image-kb-shot-01.svg)
3. Pick a preset target size, common ones are 240 KB (US DS-160), 120 KB (several EU visa services), 100 KB, 50 KB, or 20 KB (common on Indian government forms), or type your own custom number if your form's limit falls somewhere else. ![the target size presets, with a custom KB value entered](/blog/resize-image-kb-shot-02.svg)
4. Run the compression. It's not just one quality pass. The tool runs a binary search: it tries a quality level, checks how big the result is, and keeps adjusting up or down until it lands under your target. If your target is very small, like 20 KB, and quality reduction alone can't reach it, it'll shrink the pixel dimensions too. ![the tool finishing, showing the resulting file size next to your target](/blog/resize-image-kb-shot-03.svg)
5. Download the result, then open its properties in File Explorer or Finder and confirm the actual size matches before you upload it to the actual form. ![the downloaded file's size shown in File Explorer or Finder](/blog/resize-image-kb-shot-04.svg)

### Imagera's Compress Image tool

1. Go to imagera.ai and open the free **Compress Image** tool.
2. Pick a JPG, PNG, or WebP photo from your device (on a phone, this also accepts HEIC straight from the camera roll). ![picking a photo file in Imagera's compress tool](/blog/resize-image-kb-shot-05.svg)
3. Type the exact size you want into the target field, for example 100 KB. ![typing a target KB value into the size field](/blog/resize-image-kb-shot-06.svg)
4. Choose your output format. JPG is the safest default for a photo like this. ![the output format selector set to JPG](/blog/resize-image-kb-shot-07.svg)
5. Run it. The tool trims the photo to land as close to the number you typed as possible and downloads the result automatically. ![the finished file downloading, with its reported size](/blog/resize-image-kb-shot-08.svg)
...
2. Drop your photo into the upload area, or click it to browse. ![dropping a photo onto the Image Compressor's upload area](/blog/resize-image-kb-shot-09.png)
3. Once it loads, you'll see the original file size printed under the filename, and a **Quality** slider running from 1 to 100. ![the loaded image showing filename, size, and the quality slider](/blog/resize-image-kb-shot-10.png)
4. Start around 70 for a first pass. That's a reasonable middle ground for most photos. Drag the slider to set it. ![the quality slider set to around 70](/blog/resize-image-kb-shot-11.png)
5. Click **Compress**. ![clicking the Compress button](/blog/resize-image-kb-shot-12.png)
6. Check the result. The tool shows you the before and after size side by side, plus the percentage saved. ![the result panel showing original size, compressed size, and percent smaller](/blog/resize-image-kb-shot-13.png)
...
7. When the size fits your target, click **Download**. ![clicking Download on the finished compressed file](/blog/resize-image-kb-shot-14.png)
...
2. Drop in your photo. ![dropping a photo onto the Image Resizer's upload area](/blog/resize-image-kb-shot-15.png)
3. Switch to the **Percentage** tab and drag the slider down. For a passport-style photo, dropping to 50-60% of the original size is usually plenty, since these photos rarely need to be more than a few hundred pixels wide anyway. ![the percentage slider set to around 50%, with the new pixel dimensions shown underneath](/blog/resize-image-kb-shot-16.png)
4. Click **Resize & download**. ![clicking the Resize & download button](/blog/resize-image-kb-shot-17.png)
...
2. Go to **File → Export**.
3. Set the format dropdown to **JPEG**. ![the Export dialog with format set to JPEG](/blog/resize-image-kb-shot-18.svg)
4. Drag the **Quality** slider and click **Save**. ![the JPEG quality slider in the Export dialog](/blog/resize-image-kb-shot-19.svg)
5. In Finder, select the saved file and press **Cmd+I** to check its size. ![the Get Info panel showing the exported file's size](/blog/resize-image-kb-shot-20.svg)
