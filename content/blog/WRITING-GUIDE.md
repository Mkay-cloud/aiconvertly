...
- Slug: short and keyword-focused, 3-5 words and ideally under 60 characters. Build it from the article's primary target keyword in CALENDAR.md (strip stop words like "a," "the," "to," "an"), not a compressed version of the full title — a title's phrasing (a question, a colon, an "explained") carries information a slug doesn't need and just adds length. Example: title "How Video Conversion Works: MP4, WebM & MOV Explained" with target keywords "converter, convert, file converter, video format converter" becomes the slug `video-format-converter` (the most specific real keyword in the list), not `how-video-conversion-works-mp4-webm-mov-explained`.
...
- publishDate must be a quoted string (`publishDate: '2026-09-02'`), not a bare YAML date (`publishDate: 2026-09-02`). YAML parses a bare date as a native Date object, not the plain "YYYY-MM-DD" string src/lib/blog.ts's formatPublishDate expects -- it renders as "Invalid Date" on the published page. This has already broken two articles' publish-date badges after the screenshot-capture pipeline (which round-trips frontmatter through a YAML re-serialize) ran on an unquoted date.
